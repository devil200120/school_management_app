...
export const TodayAttendance = ({ todayRecord }) => {
  ...
  useEffect(() => {
    if (punchInTime) {
      ...
      const graceTime = new Date(
        expectedStart.getTime() + schoolSettings.lateGracePeriod * 60000
      );
  ...
  const handlePunchOut = async () => {
    try {
      ...
      toast.success(
        `Punched out successfully at ${formattedTime}. Total work time: ${workHours.toFixed(
          1
        )} hours`
      );
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center gap-2">
                  ...
                  {isLate && (
                    <Badge className="bg-amber-500 text-xs">Late</Badge>
                  )}
                </div>
